...
Script name: s5_reduction_eqn_survival_noexc.py

Purpose of script: Calculate Horns species reductions of survival rates not based on CL exceedances
...
# Import tdep value tables for survival and survival into input proportional rasters gdb
...
# Create output gdbs
n_rdxn_out_path = os.path.join(out_dir, 'N_survival_reduction.gdb')
if not ap.Exists(n_rdxn_out_path):
    ap.CreateFileGDB_management(out_dir, 'N_survival_reduction.gdb')

s_rdxn_out_path = os.path.join(out_dir, 'S_survival_reduction.gdb')
if not ap.Exists(s_rdxn_out_path):
    ap.CreateFileGDB_management(out_dir, 'S_survival_reduction.gdb')
...
for element in elements:
    ...
    for response_variable in response_variables:
        ...
        for spp_raster in spp_raster_list:
            ...
            out_raster_save_path = os.path.join(os.path.join(out_dir, '{}_survival_reduction.gdb'.
